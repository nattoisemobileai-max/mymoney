...
const App = () => {
  ...
  const themeColor = 'bg-emerald-500'; // 預設主題色
  ...
  const [dateRange] = useState(generateDates());
  ...
  const handleDateScroll = (direction) => {
    const newIndex = direction === 'left' ? Math.max(0, selectedDayIndex - 1) : Math.min(dateRange.length - 1, selectedDayIndex + 1);
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] text-gray-900 pb-32 font-sans select-none overflow-x-hidden">
      ...
      <main className="p-4 space-y-6">
        {activeTab === 'home' && (
          <>
            ...
            <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100/50 flex items-center gap-2">
              <button onClick={() => handleDateScroll('left')} className="p-2 text-gray-400 active:bg-gray-100 rounded-full">
                <ChevronLeft size={24} />
              </button>
              
              <div className="flex-1 flex gap-3 overflow-x-auto no-scrollbar py-2" ref={scrollRef}>
                {dateRange.map((item, idx) => (
                  <button
                    key={idx}
                    onClick={() => setSelectedDayIndex(idx)}
                    className={`flex-shrink-0 w-16 py-3 rounded-2xl border transition-all flex flex-col items-center justify-center ${
                      selectedDayIndex === idx 
                      ? 'bg-blue-500 text-white border-blue-500 shadow-lg' 
                      : 'bg-white text-gray-400 border-gray-100'
                    }`}
                  >
                    <span className="text-[9px] font-bold uppercase mb-1">DAY {idx - 14 > 0 ? idx - 14 : idx - 15}</span>
                    <span className="text-lg font-black">{item.displayDate}</span>
                    <span className="text-[10px] font-medium mt-1">{item.week}</span>
                  </button>
                ))}
              </div>

              <button onClick={() => handleDateScroll('right')} className="p-2 text-gray-400 active:bg-gray-100 rounded-full">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-red-500 p-6 rounded-[2.5rem] text-white shadow-xl shadow-red-100">
                <div className="text-xs font-bold opacity-90 mb-1 tracking-widest uppercase">總支出</div>
              ...
              <div className={`${themeColor} p-6 rounded-[2.5rem] text-white shadow-xl shadow-emerald-100`}>
                <div className="text-xs font-bold opacity-90 mb-1 tracking-widest uppercase">總收入</div>
                <div className="text-3xl font-black">$0</div>
              </div>
            </div>

            {/* 移除了原本的一語記帳與拍照記存功能塊 */}
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col items-center justify-center min-h-[200px]">
        ...
        {activeTab === 'wallet' && (
            <div className="p-4 text-center text-gray-400 font-bold">資金帳戶內容建設中...</div>
        )}
      </main>
          ...
          <div className="p-6 space-y-8 pb-20">
            ...
            <section className="flex gap-4">
              ...
              <div className="w-1/3">
                ...
                <div className="w-full bg-[#F5F5F0] p-6 rounded-2xl text-2xl font-black text-gray-400">0</div>
              </div>
            </section>
            ...
            <section className="flex gap-3 items-end">
              ...
              <button className="bg-white border-2 border-emerald-100 p-5 rounded-2xl text-emerald-500 active:scale-90 transition-transform">
                <Camera size={28} />
              </button>
            </section>
          </div>
      ...
      {/* 底部導航 - 已移除系統設定 */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t flex justify-around items-center py-5 z-40">
        <button onClick={() => setActiveTab('wallet')} className={`flex flex-col items-center w-1/3 ${activeTab === 'wallet' ? 'text-emerald-500 scale-110' : 'text-gray-400'}`}>
            <Wallet size={26}/><span className="text-[11px] mt-1 font-black">資金帳戶</span>
        </button>
